    ...
    const GenerateSummeryFromAI = async () => {
      ...
      try {
        const prompt = PROMPT.replace('{positionTitle}', resumeInfo.Experience[index].title) + 
                       " Please provide the response in a concise 5-6 point format, with each point being a clear, impactful achievement or responsibility.";
        ...
        // Limit to 6 points and map to formatted bullet points
        const formattedPoints = bulletPoints
          .slice(0, 6)
          .map(point => `• ${point}`);
        
        // Join points with newlines
        const pointwiseResponse = formattedPoints.join('\n');
        
        setValue(pointwiseResponse);
        setLoading(false);
      } catch (error) {
